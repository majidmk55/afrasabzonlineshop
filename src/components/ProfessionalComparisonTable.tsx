  ...
  return (
    <div className="mx-auto w-full max-w-[1200px] bg-white">
      ...
      <div className="border-t border-[#e8e8e8]">
        ...
        {/* RAM size selector */}
        <div className="grid border-b border-[#e8e8e8]" style={{ gridTemplateColumns: `30% 35% 35%` }}>
          <div className="flex items-center p-3 px-4 text-[#666666] text-sm">اندازه RAM</div>
          {products.slice(0, 2).map((laptop, idx) => {
            const ramSpec = laptop.specs.find(s => s.title === "حافظه رم");
            const ramSize = ramSpec?.rows.find(r => r[0] === "حافظه داخلی رم")?.[1] || "—";
            const sizeMatch = ramSize.match(/(\d+)/);
            ...
            return (
              <div key={laptop.id} className="flex items-center p-3 px-4">
                <div className="space-y-2">
                  <div className="flex items-center gap-2">
                    <div className={`flex h-4 w-4 items-center justify-center rounded-full border-2 ${
                      size === 16 ? "border-[#1a73e8] bg-[#1a73e8]" : "border-[#cccccc]"
                    }`}>
                      {size === 16 && <div className="h-1.5 w-1.5 rounded-full bg-white" />}
                    </div>
                    <span className={`text-sm ${size === 16 ? "text-[#1a73e8]" : "text-[#333333]"}`}>16GB</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <div className={`flex h-4 w-4 items-center justify-center rounded-full border-2 ${
                      size === 32 ? "border-[#1a73e8] bg-[#1a73e8]" : "border-[#cccccc]"
                    }`}>
                      {size === 32 && <div className="h-1.5 w-1.5 rounded-full bg-white" />}
                    </div>
                    <span className={`text-sm ${size === 32 ? "text-[#1a73e8]" : "text-[#333333]"}`}>32GB</span>
                  </div>
                </div>
              </div>
            );
        ...
        <div className="flex flex-col">
          ...
          <div className="grid border-b border-[#e8e8e8]" style={{ gridTemplateColumns: `30% 35% 35%` }}>
            ...
            {products.slice(0, 2).map((laptop, idx) => {
              const ramSpec = laptop.specs.find(s => s.title === "حافظه رم");
              const channels = ramSpec?.rows.find(r => r[0] === "تعداد کانال")?.[1] || "—";
              const channelMatch = channels.match(/(\d+)/);
              const channel = channelMatch ? parseInt(channelMatch[1]) : 0;
              const maxChannel = Math.max(...products.slice(0, 2).map(p => {
                const spec = p.specs.find(s => s.title === "حافظه رم");
                const ch = spec?.rows.find(r => r[0] === "تعداد کانال")?.[1] || "0";
                const match = ch.match(/(\d+)/);
                return match ? parseInt(match[1]) : 0;
              }));
              const isHighest = channel === maxChannel && channel > 0;
          ...
          <div className="grid border-b border-[#e8e8e8]" style={{ gridTemplateColumns: `30% 35% 35%` }}>
            ...
            {products.slice(0, 2).map((laptop, idx) => {
              const ramSpec = laptop.specs.find(s => s.title === "حافظه رم");
              const clockText = ramSpec?.rows.find(r => r[0] === "فرکانس")?.[1] || "—";
              const clockMatch = clockText.match(/(\d+)/);
              const clock = clockMatch ? parseInt(clockMatch[1]) : 0;
              const maxClock = Math.max(...products.slice(0, 2).map(p => {
                const spec = p.specs.find(s => s.title === "حافظه رم");
          ...
          <div className="grid border-b border-[#e8e8e8]" style={{ gridTemplateColumns: `30% 35% 35%` }}>
            ...
            {products.slice(0, 2).map((laptop, idx) => {
              const ramSpec = laptop.specs.find(s => s.title === "حافظه رم");
              const type = ramSpec?.rows.find(r => r[0] === "نوع")?.[1] || "—";
          ...
          <div className="grid border-b border-[#e8e8e8]" style={{ gridTemplateColumns: `30% 35% 35%` }}>
            ...
            {products.slice(0, 2).map((laptop, idx) => {
              ...
              const isYes = upgradable === "Yes" || upgradable === "بله";
              
              return (
                <div key={laptop.id} className={`flex items-center p-3 px-4 text-[#333333] text-sm ${isYes ? "bg-[#d4edda]" : ""}`}>
                  {upgradable}
                </div>
              );
            })}
          ...
          {/* Max. ram size */}
          <div className="grid" style={{ gridTemplateColumns: `30% 35% 35%` }}>
            <div className="flex items-center p-3 px-4 text-[#666666] text-sm">حداکثر اندازه RAM</div>
            {products.slice(0, 2).map((laptop, idx) => {
              const ramSpec = laptop.specs.find(s => s.title === "حافظه رم");
              const maxSize = ramSpec?.rows.find(r => r[0] === "حداکثر")?.[1] || "—";
              
              return (
                <div key={laptop.id} className="flex items-center p-3 px-4 text-[#333333] text-sm">{maxSize}</div>
              );
            })}
          </div>
        </div>
      ...
      <div className="border-t border-[#e8e8e8]">
        ...
        <div className="grid border-b border-[#e8e8e8]" style={{ gridTemplateColumns: `30% 35% 35%` }}>
          <div className="flex items-center p-3 px-4 text-[#666666] text-sm">اندازه ذخیره‌سازی</div>
          {products.slice(0, 2).map((laptop, idx) => {
            const storageSpec = laptop.specs.find(s => s.title === "ذخیره‌سازی");
            const storageSize = storageSpec?.rows.find(r => r[0] === "ظرفیت کلی")?.[1] || "—";
            const sizeMatch = storageSize.match(/(\d+)/);
            const size = sizeMatch ? parseInt(sizeMatch[1]) : 0;
            ...
            return (
              <div key={laptop.id} className="flex items-center p-3 px-4">
                <div className="space-y-2">
                  <div className="flex items-center gap-2">
                    <div className={`flex h-4 w-4 items-center justify-center rounded-full border-2 ${
                      size === 256 ? "border-[#1a73e8] bg-[#1a73e8]" : "border-[#cccccc]"
                    }`}>
                      {size === 256 && <div className="h-1.5 w-1.5 rounded-full bg-white" />}
                    </div>
                    <span className={`text-sm ${size === 256 ? "text-[#1a73e8]" : "text-[#333333]"}`}>256GB</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <div className={`flex h-4 w-4 items-center justify-center rounded-full border-2 ${
                      size === 512 ? "border-[#1a73e8] bg-[#1a73e8]" : "border-[#cccccc]"
                    }`}>
                      {size === 512 && <div className="h-1.5 w-1.5 rounded-full bg-white" />}
                    </div>
                    <span className={`text-sm ${size === 512 ? "text-[#1a73e8]" : "text-[#333333]"}`}>512GB</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <div className={`flex h-4 w-4 items-center justify-center rounded-full border-2 ${
                      size === 1024 ? "border-[#1a73e8] bg-[#1a73e8]" : "border-[#cccccc]"
                    }`}>
                      {size === 1024 && <div className="h-1.5 w-1.5 rounded-full bg-white" />}
                    </div>
                    <span className={`text-sm ${size === 1024 ? "text-[#1a73e8]" : "text-[#333333]"}`}>1024GB</span>
                  </div>
                </div>
              </div>
            );
        ...
        <div className="flex flex-col">
          ...
          <div className="grid border-b border-[#e8e8e8]" style={{ gridTemplateColumns: `30% 35% 35%` }}>
            ...
            {products.slice(0, 2).map((laptop, idx) => {
              const storageSpec = laptop.specs.find(s => s.title === "ذخیره‌سازی");
              const channels = storageSpec?.rows.find(r => r[0] === "کانال")?.[1] || "—";
              const channelMatch = channels.match(/(\d+)/);
              const channel = channelMatch ? parseInt(channelMatch[1]) : 0;
              const maxChannel = Math.max(...products.slice(0, 2).map(p => {
                const spec = p.specs.find(s => s.title === "ذخیره‌سازی");
                const ch = spec?.rows.find(r => r[0] === "کانال")?.[1] || "0";
                const match = ch.match(/(\d+)/);
                return match ? parseInt(match[1]) : 0;
              }));
              const isHighest = channel === maxChannel && channel > 0;
