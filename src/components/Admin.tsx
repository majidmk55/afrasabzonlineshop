...
function ProductForm({ initial, products, onSave, onClose }: { initial?: Laptop; products: Laptop[]; onSave: (p: Laptop) => void; onClose: () => void }) {
  ...
  const submit = () => {
    ...
    const p: Laptop = {
      ...
      specs: initial?.specs ?? [
        { title: "پردازنده (CPU)", icon: "cpu", rows: [["مدل", dash(f.cpu)], ["تعداد هسته", "—"], ["حداکثر فرکانس بوست", "—"]] },
        { title: "کارت گرافیک (GPU)", icon: "gpu", rows: [["مدل", dash(f.gpu)], ["حافظه ویدیویی", "—"]] },
        { title: "حافظه و ذخیره‌سازی", icon: "ram", rows: [["رم / حافظه داخلی", dash(f.mem)], ["قابلیت ارتقا", "—"]] },
        { title: "نمایشگر", icon: "display", rows: [["پنل و رزولوشن", dash(f.display)], ["نرخ نوسازی", "—"]] },
        { title: "بدنه و طراحی", icon: "scale", rows: [["وزن", dash(f.weight)], ["جنس بدنه", "—"]] },
        { title: "نرم‌افزار و گارانتی", icon: "shield", rows: [["گارانتی", "۲ سال کورهِوس + گارانتی رسمی"], ["اقلام همراه", "شارژر و کابل اصلی"]] },
      ],
  ...
  const parsed = useMemo(() => {
    ...
    for (const line of text.split("\n")) {
      ...
      ok.push({
        ...
        specs: [
          { title: "پردازنده (CPU)", icon: "cpu", rows: [["مدل", "به‌زودی"]] },
          { title: "حافظه و ذخیره‌سازی", icon: "ram", rows: [["رم / حافظه داخلی", "به‌زودی"]] },
          { title: "نمایشگر", icon: "display", rows: [["پنل و رزولوشن", "به‌زودی"]] },
          { title: "نرم‌افزار و گارانتی", icon: "shield", rows: [["گارانتی", "۲ سال کورهِوس + گارانتی رسمی"]] },
        ],
