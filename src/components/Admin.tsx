      ...
      <div className="dark-panel font-nazanin flex min-h-screen items-center justify-center p-4">
        ...
        >
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="font-nazanin text-xl font-bold text-paper">پنل مدیریت افرالینک</p>
...
function ProductForm({ initial, products, onSave, onClose }: { initial?: Laptop; products: Laptop[]; onSave: (p: Laptop) => void; onClose: () => void }) {
  const [f, setF] = useState({
    ...
    warrantyCompany: initial?.warrantyCompany ?? "افرالینک",
  });
  ...
  const submit = () => {
    ...
    const p: Laptop = {
      ...
      tagline: f.tagline.trim() || "جدیدترین ورودی فروشگاه افرالینک با کارنامه آزمایشگاه.",
      ...
      specs: initial?.specs ?? [
        ...
        { title: "صفحه نمایش", icon: "display", rows: [["اندازه صفحه نمایش", dash(f.display)], ["فناوری و نوع صفحه", "—"], ["نسبت تصویر (Aspect Ratio)", "—"], ["نسبت صفحه به بدنه (Screen-to-Body)", "—"], ["نوع سطح صفحه", "—"], ["پرداخت سطح (Finishing)", "—"], ["زمان پاسخ", "—"], ["رزولوشن", "—"], ["کلاس رزولوشن", "—"], ["تراکم پیکسلی (Pixel Density)", "—"], ["فاصله پیکسل (Pixel Pitch)", "—"], ["نرخ نوسازی (Refresh Rate)", "—"], ["اندازه روشنایی", "—"], ["پشتیبانی HDR", "—"], ["نسبت کنتراست (Contrast Ratio)", "—"], ["تعداد رنگ", "—"], ["صفحه نمایش لمسی", "—"], ["عمق رنگ (Color Depth)", "—"], ["استانداردهای HDR", "—"]] },
        ...
        { title: "حافظه رم", icon: "ram", rows: [["حافظه داخلی رم", dash(f.mem)], ["نوع حافظه", "—"], ["مدل حافظه رم", "—"], ["فرم فاکتور (Form Factor)", "—"], ["تعداد اسلات‌ها و کانال‌های رم", "—"]] },
        { title: "ذخیره‌سازی", icon: "drive", rows: [["مدل و نوع حافظه", "—"], ["ظرفیت کلی", "—"], ["فرم فاکتور (Form Factor)", "—"], ["رابط SSD", "—"], ["نسخه NVMe", "—"], ["سرعت خوانش و نوشتن", "—"], ["نوع و نسل NAND", "—"], ["تعداد لایه‌های NAND (NAND Layer)", "—"], ["تعداد اسلات‌ها", "—"], ["عمر نوشتاری (TBW)", "—"], ["میانگین زمان بین خرابی (MTBF)", "—"]] },
        ...
        { title: "نرم‌افزار", icon: "info", rows: [["سیستم‌عامل نصب‌شده", "Windows 11"], ["گارانتی", "۲ سال افرالینک + گارانتی رسمی"]] },
      ...
      inBox: initial?.inBox ?? ["خود دستگاه", "شارژر اصلی", "کابل برق", "دفترچه راهنما", "گزارش آزمایشگاه افرالینک"],
      pros: initial?.pros ?? ["جدیدترین ورودی فروشگاه افرالینک", "دارای کارنامه آزمایشگاه"],
      cons: initial?.cons ?? ["هنوز دیدگاه ثبت نشده است"],
      warrantyCompany: f.warrantyCompany.trim() || "افرالینک",
    ...
    <div className="fixed inset-0 z-50 overflow-y-auto" role="dialog" aria-modal="true">
      ...
      <div className="panel-in relative mx-auto my-8 w-[min(720px,94vw)] border border-line bg-white p-6 shadow-2xl sm:p-8">
        ...
        <div className="mt-5 grid gap-4 sm:grid-cols-2">
          ...
          <div className="sm:col-span-2">
            <Field label="شرکت گارانتی‌کننده"><input className={inputCls} value={f.warrantyCompany} onChange={(e) => set("warrantyCompany", e.target.value)} placeholder="افرالینک" /></Field>
          </div>
        </div>
  ...
  const parsed = useMemo(() => {
    ...
    for (const line of text.split("\n")) {
      ...
      ok.push({
        ...
        tagline: "ورودی تازه فروشگاه افرالینک — به‌زودی با کارنامه کامل آزمایشگاه.",
        highlights: [brand, category || "اولترابوک"],
        pros: ["جدیدترین ورودی فروشگاه افرالینک"],
        ...
        specs: [
          ...
          { title: "صفحه نمایش", icon: "display", rows: [["اندازه صفحه نمایش", "به‌زودی"], ["فناوری و نوع صفحه", "—"], ["رزولوشن", "—"], ["نرخ نوسازی (Refresh Rate)", "—"], ["صفحه نمایش لمسی", "—"]] },
          ...
          { title: "حافظه رم", icon: "ram", rows: [["حافظه داخلی رم", "به‌زودی"], ["مدل حافظه رم", "—"], ["فرم فاکتور (Form Factor)", "—"], ["تعداد اسلات‌ها و کانال‌های رم", "—"]] },
        ...
        inBox: ["خود دستگاه", "شارژر اصلی", "گزارش آزمایشگاه افرالینک"],
        active: true,
      });
    }
    return { ok, skipped };
